...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_festival_id') {
    ...
    $eventYear  = trim($_POST['event_Year']  ?? '');
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO lineups_list (event_Name, event_Year, event_StartDate, event_EndDate) VALUES (?, ?, ?, ?)");
        $stmt->execute([
            $eventName,
...
if (isset($_GET['action']) && $_GET['action'] === 'setlist_data') {
    ...
    try {
        $stmt = $pdo->prepare(
            "SELECT valid_days, stage_format, festival_name
        ...
        if (!$logic) {
            echo json_encode(['error' => "No lineups_import_logic record found for festival ID {$fid}."]);
            exit;
        }
        ...
        $stmt2 = $pdo->prepare(
            "SELECT * FROM vw_lineups_transactions_detail
              WHERE festival_ID = :fid ORDER BY day_ord, start_minutes"
        );
        ...
        // 3. Preferences
        $stmt3 = $pdo->prepare(
            "SELECT trans_ID, viewer, want, need FROM lineups_preferences WHERE festival_ID = :fid"
        );
        ...
        foreach ($stmt3->fetchAll() as $pref) {
            ...
            if ($type) {
                $prefsByTrans[(int)$pref['trans_ID']][$pref['viewer']] = $type;
        ...
        foreach ($transactions as $tx) {
            ...
            if (isset($schedule[$txDay][$txStage])) {
                $row = [
                    ...
                    'schedule'      => (int)($tx['schedule'] ?? 0),
                    'start_minutes' => $tx['start_minutes'] ?? 0,
                    'prefs'         => $prefsByTrans[(int)$tx['ID']] ?? [],
                ];
        ...
        foreach ($schedule as $day => &$stages) {
            foreach ($stages as $stage => &$bands) {
                usort($bands, fn($a, $b) => $b['start_minutes'] <=> $a['start_minutes']);
            }
        }
        unset($stages, $bands);
...
$festivals = [];
try {
    $festivals = $pdo->query("SELECT * FROM vw_lineups_festival_list")->fetchAll();
...
<div class="festivals-wrap">
      ...
      <div class="table-scroll">
        <table class="festival-table">
          ...
          <tbody>
            <?php foreach ($festivals as $f): ?>
            <tr>
              <td><span class="badge-id"><?= htmlspecialchars($f['id']) ?></span></td>
  ...
  <div class="tab-panel" id="panel-setlist" role="tabpanel">
    <?php include 'SetListViewer.php'; ?>
  </div>

  <!-- ── Tab 4: Lockscreen ── -->
  <div class="tab-panel" id="panel-lockscreen" role="tabpanel">
    <?php include 'lockscreen_partial.php'; ?>
  </div>
...
<div class="modal-overlay" id="addFestModal">
  <div class="fest-modal">
    <h3>Add Festival</h3>
    <p class="modal-subtitle">Enter the details for the new festival.</p>

    <div class="modal-field">
      <label for="newEventName">Event Name</label>
      <input type="text" id="newEventName" placeholder="e.g. Coachella" autocomplete="off">
    </div>

    <div class="modal-field">
      <label for="newEventYear">Event Year</label>
      <input type="number" id="newEventYear" placeholder="e.g. 2025" autocomplete="off">
    </div>
    ...
    <div class="modal-actions">
      ...
      <button class="btn-modal-save"   id="saveBtn">Save</button>
    </div>
  </div>
</div>
...
(function () {
  ...
  const yearInput     = document.getElementById('newEventYear');
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    if (!eventName) {
      hint.textContent = 'Event name is required.';
    ...
    try {
      ...
      form.append('event_Year',       yearInput.value.trim());
      form.append('event_StartDate',  startInput.value);
      form.append('event_EndDate',    endInput.value);

      const res  = await fetch('index.php', { method: 'POST', body: form });
      const data = await res.json();

      if (data.success) {
        toast('Festival added successfully.', 'success');
        closeModal();
