...
if (isset($_GET['action']) && $_GET['action'] === 'setlist_data') {
    ...
    try {
        ...
        // 2. Transactions
        $stmt2 = $pdo->prepare(
            "SELECT * FROM lineups_transactions
...
$festivals = [];
try {
    $festivals = $pdo->query("SELECT id, event_Name, event_Year, event_StartDate, event_EndDate FROM lineups_list ORDER BY event_StartDate")->fetchAll();
