...
// HTML partial for the Set List tab panel in SetListViewer.php.
// Included inside SetListViewer.php; no PHP data fetching done here.
// All set list data is loaded client-side via AJAX (SetListViewer.php?action=setlist_data).
// Listens for the 'festivalChanged' custom event dispatched by SetListViewer.php.
// ─────────────────────────────────────────────────────────────────────────
?>
...
#panel-setlist {
    ...
    --tl-ppm:    1.5px;
    /* --tl-height is set dynamically per day based on actual schedule times */
...
.sl-band-card:hover { z-index: 10; overflow: visible; background: var(--sl-surface); }
.sl-band-name {
    font-family: 'Barlow Condensed', sans-serif;
    font-size: 13px;
    font-weight: 700;
...
.sl-star-btn:hover { opacity: 0.7; transform: scale(1.15); }
.sl-star-btn.starred { opacity: 1; }
.sl-star-btn.starred svg { fill: #ffd740; }
.sl-star-btn.saving { pointer-events: none; opacity: 0.4; }
...
(function () {
    ...
    // Populated per-day after data loads: { "Friday": { start, end }, … }
    let dayRanges = {};

    // Scan all entries for a single day's schedule and return { start, end }
    // rounded to the nearest 15-minute boundary (floor for start, ceil for end).
    function computeDayRange(daySchedule) {
        let minStart = Infinity;
        let maxEnd   = -Infinity;
        Object.values(daySchedule).forEach(txList => {
            txList.forEach(tx => {
                const s = timeToMins(tx.start_Time);
                const e = timeToMins(tx.end_Time);
                if (s !== null && s < minStart) minStart = s;
                if (e !== null) {
                    // Adjust for overnight: end before start means it crosses midnight
                    const adjE = (e < s) ? e + 1440 : e;
                    if (adjE > maxEnd) maxEnd = adjE;
                }
            });
        });
        // Fallback if no valid times found
        if (!isFinite(minStart)) minStart = 690;   // 11:30 AM
        if (!isFinite(maxEnd))   maxEnd   = 1500;  // 1:00 AM

        // Round start DOWN to nearest 15 min, end UP to nearest 15 min
        const start = Math.floor(minStart / 15) * 15;
        const end   = Math.ceil(maxEnd   / 15) * 15;
        return { start, end };
    }
    ...
    function render(data) {
        ...
        days.forEach((day, dayIdx) => {
            ...
            dayRanges[day] = computeDayRange(daySchedule);
            ...
            ruler.style.setProperty('--tl-height', ((dayRanges[day].end - dayRanges[day].start) * PPM) + 'px');
            ...
            stageNames.forEach(stageName => {
                ...
                const { start: tlS, end: tlE } = dayRanges[day];
                body.style.setProperty('--tl-height', ((tlE - tlS) * PPM) + 'px');
                ...
                (daySchedule[stageName] || []).forEach(tx => {
                    ...
                    Object.assign(card.dataset, {
                        ...
                        transId:  tx.id || '',
                        schedule: tx.schedule || 0,
                    });

                    let html = '';
                    // Star button — top-left corner
                    const isStarred = parseInt(tx.schedule) === 1;
                    html += `<button class="sl-star-btn${isStarred ? ' starred' : ''}" title="${isStarred ? 'Remove from schedule' : 'Add to schedule'}" aria-label="Toggle schedule" aria-pressed="${isStarred}">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="${isStarred ? '#ffd740' : 'none'}" stroke="#ffd740" stroke-width="2" stroke-linejoin="round">
                    ...
                    // Star click handler
                    card.querySelector('.sl-star-btn').addEventListener('click', (e) => {
                        e.stopPropagation();
                        toggleSchedule(card);
                    });
    ...
    // ── Star / schedule toggle ──────────────────────────────────────────────
    async function toggleSchedule(card) {
        const transId   = card.dataset.transId;
        const festId    = currentFestId;
        if (!transId || !festId) return;

        const btn       = card.querySelector('.sl-star-btn');
        const current   = parseInt(card.dataset.schedule) === 1;
        const newValue  = current ? 0 : 1;

        // Optimistic UI update
        btn.classList.add('saving');
        btn.classList.toggle('starred', !!newValue);
        btn.title        = newValue ? 'Remove from schedule' : 'Add to schedule';
        btn.setAttribute('aria-pressed', String(!!newValue));
        const svgFill    = newValue ? '#ffd740' : 'none';
        const svg        = btn.querySelector('svg');
        if (svg) svg.setAttribute('fill', svgFill);
        card.dataset.schedule = newValue;

        try {
            const form = new FormData();
            form.append('action',      'set_schedule');
            form.append('festival_id', festId);
            form.append('trans_id',    transId);
            form.append('schedule',    newValue);

            const res  = await fetch('api/lineups_api.php', { method: 'POST', body: form });
            const data = await res.json();

            if (!data.success) {
                // Revert on failure
                const reverted = newValue ? 0 : 1;
                card.dataset.schedule = reverted;
                btn.classList.toggle('starred', !!reverted);
                btn.title = reverted ? 'Remove from schedule' : 'Add to schedule';
                btn.setAttribute('aria-pressed', String(!!reverted));
                if (svg) svg.setAttribute('fill', reverted ? '#ffd740' : 'none');
                console.error('set_schedule failed:', data.error);
            } else {
                // Notify lockscreen tab to refresh
            ...
            btn.setAttribute('aria-pressed', String(!!reverted));
            if (svg) svg.setAttribute('fill', reverted ? '#ffd740' : 'none');
            console.error('toggleSchedule network error:', err);
        } finally {
    ...
    function positionCards() {
        document.querySelectorAll('.sl-band-card').forEach(card => {
            ...
            // Find which day this card belongs to by walking up to the day panel
            const panel = card.closest('.sl-day-panel');
            const day   = panel ? panel.id.replace('sl-panel-', '') : null;
            const { start: tlStart, end: tlEnd } = (day && dayRanges[day]) ? dayRanges[day] : { start: 690, end: 1500 };

            card.style.height = Math.max((adjEnd - start) * PPM - 2, 52) + 'px';
            card.style.top    = (tlEnd - adjEnd) * PPM + 'px';
        });
    }
    ...
    function buildRulers() {
        currentDays.forEach(day => {
            ...
            if (!ruler) return;
            ruler.innerHTML = '';
            const { start: tlStart, end: tlEnd } = dayRanges[day] || { start: 690, end: 1500 };
            for (let m = tlStart; m <= tlEnd; m += 60) {
                const tick = document.createElement('div');
                tick.className = 'sl-time-tick';
                tick.style.top = (tlEnd - m) * PPM + 'px';
    ...
    async function loadFestival(festivalId) {
        ...
        try {
            const res  = await fetch(`index.php?action=setlist_data&festival_id=${encodeURIComponent(festivalId)}`);
            const data = await res.json();
